...
\copy stg.rad_data_100 from '/home/user/PostRep/Upscaled_data/new_test_100.csv' delimiter ',' csv header


ALTER TABLE stg.rad_data_100 ADD probe_time timestamp without time zone;


UPDATE stg.rad_data_100 SET probe_time = (date|| ' ' || time)::timestamp without time zone;


ALTER TABLE stg.rad_data_100
DROP COLUMN time;
...
DROP TABLE IF EXISTS proc.station_info_100;


CREATE TABLE proc.station_info_100 (
    id int primary key,
    place text,
...
DROP TABLE IF EXISTS proc.records_info_100;


CREATE TABLE proc.records_info_100 (
    id int primary key ,
    station_id int references proc.station_info_100(id),
    place text,
...
INSERT INTO proc.station_info_100 (id, place, geom, geom_3d) (
    ...
    FROM
      (SELECT DISTINCT ON (place, geom)
            ...
            geom,
            geom_3d
       FROM stg.rad_data_100) AS foo);


INSERT INTO proc.records_info_100 (id, place, probe_time, value, station_id)
  ( SELECT a.id,
           a.place,
           a.probe_time,
           a.val::real,
           b.id
   FROM stg.rad_data_100 AS a
   LEFT JOIN proc.station_info_100 AS b ON a.place = b.place
   AND a.geom = b.geom);


CREATE INDEX ON proc.station_info_100  USING gist(geom);

CREATE INDEX ON proc.station_info_100  USING gist(geom_3d);
...
SELECT inData.id AS id,
	...
	FROM (
			...
			proc.station_info_100 AS inData
...
SELECT inData.id AS id,
  ...
  SELECT 	vor.id as id,
    ...
    vor.geom as geom
  FROM proc.voronoi_clip_100 as vor
  LEFT JOIN proc.records_info_100 as rec
